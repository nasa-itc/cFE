...
void Test_SB_Cmds_RstCtrs(void)
{
    ...
    UT_CallTaskPipe(CFE_SB_ProcessCmdPipePkt,
                    CFE_MSG_PTR(ResetCounters.SBBuf),
                    sizeof(ResetCounters.Cmd),
...
void Test_SB_Cmds_Stats(void)
{
    ...
    /* Expect 4 more events generated: 1 for send stats + 1 for each pipe cleanup */
    CFE_UtAssert_EVENTCOUNT(6); /* jphfix - clear events here? */
    ...
    memset(&SendSbStats, 0, sizeof(SendSbStats));
    UT_ClearEventHistory(); /* start over with the event count */
    ...
    /* There should be 1 event for each pipe, plus 2 due to sending stats w/ no subscribers */
    CFE_UtAssert_EVENTCOUNT(CFE_MISSION_SB_MAX_PIPES + 2);
...
void Test_SB_Cmds_RoutingInfoDef(void)
{
    ...
    UT_CallTaskPipe(CFE_SB_ProcessCmdPipePkt,
                    CFE_MSG_PTR(WriteRoutingInfo.SBBuf),
                    sizeof(WriteRoutingInfo.Cmd),
...
void Test_SB_Cmds_PipeInfoDef(void)
{
    ...
    UT_CallTaskPipe(CFE_SB_ProcessCmdPipePkt,
                    CFE_MSG_PTR(WritePipeInfo.SBBuf),
                    sizeof(WritePipeInfo.Cmd),
...
void Test_SB_Cmds_MapInfoDef(void)
{
    ...
    UT_CallTaskPipe(CFE_SB_ProcessCmdPipePkt,
                    CFE_MSG_PTR(WriteMapInfo.SBBuf),
                    sizeof(WriteMapInfo.Cmd),
                    UT_TPID_CFE_SB_CMD_WRITE_MAP_INFO_CC);
    UtAssert_UINT8_EQ(CFE_SB_Global.HKTlmMsg.Payload.CommandCounter, 1);
...
void Test_SB_Cmds_EnRouteValParam(void)
{
    ...
    UT_CallTaskPipe(CFE_SB_ProcessCmdPipePkt,
                    CFE_MSG_PTR(EnableRoute.SBBuf),
                    sizeof(EnableRoute.Cmd),
...
void Test_SB_Cmds_SendPrevSubs(void)
{
    ...
    /* Bad Size */
    UT_CallTaskPipe(CFE_SB_ProcessCmdPipePkt,
                    CFE_MSG_PTR(SendPrevSubs.SBBuf),
                    0,
...
void Test_SB_Cmds_SubRptOn(void)
{
    ...
    UT_CallTaskPipe(CFE_SB_ProcessCmdPipePkt,
                    CFE_MSG_PTR(EnableSubReporting.SBBuf),
                    sizeof(EnableSubReporting.Cmd),
...
void Test_SB_Cmds_SubRptOff(void)
{
    ...
    UT_CallTaskPipe(CFE_SB_ProcessCmdPipePkt,
                    CFE_MSG_PTR(DisableSubReporting.SBBuf),
                    sizeof(DisableSubReporting.Cmd),
...
void Test_GetPipeName_API(void)
{
    ...
    SB_UT_ADD_SUBTEST(Test_GetPipeName_NullPipeName);
...
void Test_GetPipeIdByName(void)
{
    ...
    UT_SetDataBuffer(UT_KEY(OS_QueueGetIdByName),
                     &(CFE_SB_Global.PipeTbl[0].SysQueueId),
                     sizeof(CFE_SB_Global.PipeTbl[0].SysQueueId),
                     false);
...
void Test_Unsubscribe_Local(void)
{
    ...
    CFE_UtAssert_SUCCESS(CFE_SB_UnsubscribeLocal(MsgId, TestPipe));
...
void Test_MessageTxn_GetEventDetails(void)
{
    ...
    memset(EvtMsg, 0xAA, sizeof(EvtMsg));
    UtAssert_VOIDCALL(CFE_SB_MessageTxn_GetEventDetails(&Txn,
                                                        NULL,
...
static int32
UT_CheckSetSequenceCount(void *UserObj, int32 StubRetcode, uint32 CallCount, const UT_StubContext_t *Context)
{
